...
const convertToProjectModels = (data) => {
    let GpuJbl = data.data.map(convertToProjectModel)
    return GpuJbl;
}
...
const JblAPI = {
    ...
    put(Jbl){
        return fetch(`${url}/${Jbl.id}`, {
            method: "PUT",
            body: JSON.stringify(Jbl),
            headers: {
    ...
    post(data) {
        return fetch(url)
            ...
            .catch((error) => {
                console.log("Log client error: " + error);
                throw new Error("There was an error fetching existing data. Please try again");
            });
    },
    delete(id) {
        return fetch(`${url}/${id}`,{
